...
exports.registerUser = catchAsyncError(async (req, res, next) => {
    ...
    if (isEmail) {
        ...
        const message = `Welcome to TeleConnect ${name}`;
        try {
            // Check if contact already has a country code (starts with +)
            // If not, default to +91 or just use as is (depending on your preference, here using as is for flexibility)
            ...
            // return next(new ErrorHander(error.message, 500)); // Commented out to prevent blocking
        }
    }
    ...
    try {
        ...
        return next(new ErrorHander("Token generation failed", 500));
    }
});
